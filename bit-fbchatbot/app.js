...
var options = {
  key: fs.readFileSync("/home/ec2-user/vars/custom.key"),
	cert: fs.readFileSync("/home/ec2-user/vars/www_kjhak_xyz.crt"),
	ca: fs.readFileSync("/home/ec2-user/vars/www_kjhak_xyz.ca-bundle")
}
...
function receivedMessage(event) {
  ...
  if (messageText) {
    ...
    // and send back the template example. Otherwise, just echo the text we received.
	  ...
	  // 기존 case 문을 if 문으로 변경하였다.
	  // 챗봇으로 사칙연산을 구현하기 위해 "op:" 라는 명령어를 등록하였다.
	  // 사용자로부터 "op:" 문자열이 넘어 오면 사칙연산을 수행하는 코드를 추가한다. 
    ...
    request({
      ...
      if (!error && response.statusCode == 200) {
        ...
        if (obj.state == "success") {
          ...
          sendTextMessage(senderID, "전구 제어 실패");
        }
      } else {
        console.log("SbringBoot 요청 오류", body)
      }
    })
